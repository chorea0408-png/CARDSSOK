...
const RATIO_BUTTONS: { ratio: SlideRatio; icon: string }[] = [
  { ratio: '1:1',  icon: '□' },
  { ratio: '4:5',  icon: '▭' },
  { ratio: '9:16', icon: '▯' },
];
    ...
    <header className="h-14 border-b border-gray-200 bg-white px-4 flex items-center justify-between shrink-0 z-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
          {RATIO_BUTTONS.map(({ ratio, icon }) => (
            <button
              key={ratio}
              onClick={() => setSlideRatio(ratio)}
              title={SLIDE_SIZE_PRESETS[ratio].label}
              className={clsx(
                'flex flex-col items-center justify-center px-2.5 py-1 rounded-md text-xs font-semibold transition-all gap-0.5',
                slideRatio === ratio
